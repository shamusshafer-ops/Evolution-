...
|20| Season UI indicator (current phase readout) | Deferred from M4 for time; mechanism works, just no visible readout of where in the cycle a run is | S | **Shipped 2026-08-02** — Population and both graph axes show resource-cycle direction, exact food multiplier, and day/night phase where enabled |
...
|44| Field Notebook and causal inspection | Persistent events, organism energy/life history, lineage follow, paired comparisons, and honest evidence language | M | **Four slices shipped 2026-08-02** — durable event/follow-up evidence, inspector, scenario groups, manual/batch comparison, graph markers, stable census filtering, phase labels, and map lineage follow; historical lineage trait ribbons and generalized experiments remain |
